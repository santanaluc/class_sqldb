...
--Excluindo chave estrangeira(relação entre uma tabela e outra) da tabela vendaProd
alter table vendaProd drop constraint [FK__vendaProd__cd_ve__3A81B327] -- FK excluida

--Excluindo a chave primária da tabela venda
alter table venda drop constraint [PK__venda__8C6EA002DD3156E2] -- PK excluída

--Recriando chave primária da tabela venda
...
--Criando chave primária da tabela produto
...
-- Adicionar um check na tabela produto para o vl_produto == adicionar uma restrição 
alter table produto add constraint CK_vlProduto check (vl_produto > 5) -- já com o nome definido
